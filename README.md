# GATE DA Preparation Workspace

The study material and sprint plan were built from the two supplied syllabus PDFs:

- [GATE DA complete study material](GATE_DA_Complete_Study_Material.md)
- [Expanded coverage supplement](GATE_DA_Expanded_Coverage_Supplement.md)
- [2024–2026 PYQ solutions](GATE_DA_PYQ_Solutions.md)
- [Exam-weighted preparation strategy](GATE_DA_Exam_Weighted_Strategy.md)
- [176-day execution plan](GATE_DA_170_Day_Plan.md)
- [DA syllabus PDF](DA_GATE2027_Syllabus.pdf)
- [GA syllabus PDF](GA_GATE2027_Syllabus.pdf)

Recommended order:

1. Read the roadmap and start the current week in the 176-day plan.
2. Use the complete study material for the topic being studied.
3. Solve timed questions and record every error.
4. Revisit the formula sheet and weak topics on the weekly review day.

The PDFs are treated as the syllabus source. Verify the official GATE brochure for the relevant year for paper pattern, dates, marks, and admission rules.

## PDF releases

The workflow in .github/workflows/release-pdfs.yml checks out the pinned md-latex-mermaid2pdf converter, renders every Markdown file with headless Chromium, and uploads the resulting PDFs to GitHub Releases. Trigger it by pushing a tag such as v1.0.0, or run it manually with a release tag such as pdf-2027-01.
